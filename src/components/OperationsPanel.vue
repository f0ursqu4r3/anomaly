<template>
  <div class="operations-panel">
    ...
    <div class="directive-list">
      ...
      >
        <SvgIcon :name="d.icon" size="sm" class="dir-icon" />
    ...
    <div v-for="platform in platforms" :key="platform.id" class="platform-row">
      <button class="platform-header" @click="togglePlatform(platform.id)">
        ...
        <span class="platform-cargo mono">
          {{ loaded(platform) }}/{{ getState(platform)?.capacity ?? 100 }}
        </span>
        <span class="platform-state" :class="statusClass(platform)">
          ...
          <div class="cargo-section">
            ...
            <div v-if="loaded(platform) > 0" class="cargo-breakdown">
              <span v-if="getState(platform)!.cargo.metals > 0" class="cargo-item">
                {{ Math.round(getState(platform)!.cargo.metals) }}m
              </span>
              <span v-if="getState(platform)!.cargo.ice > 0" class="cargo-item">
                {{ Math.round(getState(platform)!.cargo.ice) }}i
              </span>
              <span v-if="getState(platform)!.cargo.rareMinerals > 0" class="cargo-item">
                {{ Math.round(getState(platform)!.cargo.rareMinerals) }}r
              </span>
...
.status-dot.status-docked {
  background: var(--green);
  box-shadow: 0 0 4px var(--green-glow);
}
.status-dot.status-transit {
  background: var(--amber);
  box-shadow: 0 0 4px var(--amber-glow);
}
.status-dot.status-returning {
...
.platform-state.status-docked {
  color: var(--green);
}
.platform-state.status-transit {
  color: var(--amber);
}
.platform-state.status-returning {
  color: var(--text-muted);
}
